...
def convert_onnx_to_igie(igie_model, input_dict, onnx_model, fp16):
    import onnx 
    ...
    opt_onnx_model = onnx_model.replace(".onnx", ".opt.onnx")
    if not os.path.exists(opt_onnx_model):
        onnx_mod = onnx.load(onnx_model)
        onnx_mod, _ = simplify(onnx_mod)
        onnx.save(onnx_mod, opt_onnx_model)
    else:
        onnx_mod = onnx.load(opt_onnx_model)
    

    relax_mod = from_onnx(onnx_mod, shape_dict=input_dict)
    ...
    ex = relax.build(relax_mod, target=target, precision=precision)
    ex.export_library(igie_model)
    logging.info("Succesfully convert onnx to igie...")
