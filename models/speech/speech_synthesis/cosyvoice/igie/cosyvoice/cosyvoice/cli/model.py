...
from cosyvoice.utils.file_utils import convert_onnx_to_trt, convert_onnx_to_igie, export_cosyvoice2_vllm
from cosyvoice.utils.common import IgieContextWrapper, OrtContextWrapper, TrtContextWrapper
...
class CosyVoiceModel:
    ...
    def load_onnx(self, flow_decoder_onnx_model):
        assert os.path.exists(flow_decoder_onnx_model), '{} not found!'.format(flow_decoder_onnx_model)
        del self.flow.decoder.estimator
        self.flow.decoder.estimator = OrtContextWrapper(flow_decoder_onnx_model, device=self.device)
    ...
    def load_igie(self, flow_decoder_estimator_model, flow_decoder_onnx_model, fp16):
        del self.flow.decoder.estimator
        if not os.path.exists(flow_decoder_estimator_model) or os.path.getsize(flow_decoder_estimator_model) == 0:
            convert_onnx_to_igie(flow_decoder_estimator_model, self.get_igie_kwargs(), flow_decoder_onnx_model, fp16)
        self.flow.decoder.estimator = IgieContextWrapper(flow_decoder_estimator_model)

    def get_igie_kwargs(self):
        from tvm import tir

        seq_len = tir.Var("seq_len", "int64")
        input_dict = {"x": [2, 80, seq_len], "mask": [2, 1, seq_len], "mu": [2, 80, seq_len], "t": [2], "spks": [2, 80], "cond": [2, 80, seq_len]}
