...
class CosyVoice:
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_onnx=False, fp16=False, trt_concurrent=1):
        assert not (load_trt and load_onnx), 'load_trt and load_onnx cannot be enabled at the same time'
...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_onnx=False, load_igie=False, load_vllm=False, fp16=False, trt_concurrent=1):
        assert sum([load_trt, load_onnx, load_igie]) <= 1, 'load_trt/load_onnx/load_igie cannot be enabled at the same time'
        self.model_dir = model_dir
        self.fp16 = fp16
        if not os.path.exists(model_dir):
            model_dir = snapshot_download(model_dir)
        ...
        if load_onnx:
            self.model.load_onnx('{}/flow.decoder.estimator.fp32.onnx'.format(model_dir))
        if load_igie:
            self.model.load_igie('{}/flow.decoder.estimator.{}.so'.format(model_dir, 'fp16' if self.fp16 is True else 'fp32'),
                                '{}/flow.decoder.estimator.fp32.onnx'.format(model_dir),
                                self.fp16)
        del configs
...
class CosyVoice3(CosyVoice2):
    ...
    def __init__(self, model_dir, load_trt=False, load_vllm=False, load_onnx=False, fp16=False, trt_concurrent=1):
        assert not (load_trt and load_onnx), 'load_trt and load_onnx cannot be enabled at the same time'
        self.model_dir = model_dir
        self.fp16 = fp16
        if not os.path.exists(model_dir):
