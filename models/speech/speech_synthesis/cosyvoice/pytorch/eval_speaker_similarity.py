...
def main():
    ...
    def load_wav(wav_file, obj_fs=16000):
        if ".ark:" not in wav_file:
            # torchaudio 2.9+ delegates decoding to TorchCodec. Read regular
            # audio files with soundfile so TorchCodec is not required.
            wav, fs = sf.read(wav_file, dtype="float32", always_2d=True)
        ...
        if fs != obj_fs:
            # torchaudio.sox_effects was removed in torchaudio 2.10.
    ...
    if args.ref_wavs is not None and len(args.ref_wavs) > 0 and os.path.exists(args.ref_wavs):
        with open(args.ref_wavs, "rt") as ref_file:
            for line in ref_file:
                uttid, wav_path = line.strip().split(maxsplit=1)
                ref_wavs[uttid] = wav_path
    prompt_wavs = {}
    with open(args.prompt_wavs, "rt") as prompt_file:
        for line in prompt_file:
            uttid, wav_path = line.strip().split(maxsplit=1)
            prompt_wavs[uttid] = wav_path
    hyp_wavs = []
    with open(args.hyp_wavs, "rt") as hyp_file:
    ...
    for uttid, hyp_wav_path in hyp_wavs:
        ...
        try:
            ...
            print(f"Warning: failed to score {uttid}: {exc}")
            continue

    if not hyp_scores:
        out_file.close()
        raise RuntimeError("No speaker similarity scores were produced; check the warnings above.")
