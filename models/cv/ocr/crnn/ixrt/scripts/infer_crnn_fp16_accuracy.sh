...
ORG_MODEL=${CHECKPOINTS_DIR}/${MODEL_NAME}.onnx
ORG_MODEL_32=${CHECKPOINTS_DIR}/${MODEL_NAME}_org_${BSZ}.onnx
# Change Batchsize
let step++
echo;
...
if [ -f $ORG_MODEL_32 ];then
    echo "  "Change Batchsize Skip, $ORG_MODEL_32 has been existed
else
    python3 ${RUN_DIR}/modify_batchsize.py --batch_size ${BSZ} \
        --origin_model ${ORG_MODEL} --output_model ${ORG_MODEL_32}
    echo "  "Generate ${ORG_MODEL_32}
fi
