...
def get_args_parser(
    ...
    parser.add_argument(
        ...
        default=False,
        help="Only benchmark the TVM engine FPS without loading datasets",
    )
    parser.set_defaults(
...
def run_perf_only(model):
    if not hasattr(model, "benchmark"):
        raise ValueError("--perf_only is only supported by the TVM backend (*.so)")
    mean_time_ms, fps = model.benchmark(number=100, repeat=1)
    print(f"\n* Mean inference time: {mean_time_ms:.3f} ms, Mean fps: {fps:.3f}")
    logger.info("Mean inference time: %.3f ms, Mean fps: %.3f", mean_time_ms, fps)
