...
from calibration_dataset import TransformImage, getdataloader
...
def main(config):
    ...
    dataloader = getdataloader(config.datasets_dir, val_tf, config.loop_count, config.bsz, img_sz=config.imgsz)
    ...
    if config.test_mode == "FPS":
        ...
        with tqdm(total= len(dataloader)) as _tqdm:
            for idx, (batch_data, batch_label) in enumerate(dataloader):
                ...
                batch_top1, batch_top5 = eval_batch(output.numpy(), batch_label)
                acc_top1 += batch_top1
                acc_top5 += batch_top5
...
def parse_config():
    ...
    parser.add_argument("--bsz", type=int, default=64, help="test batch size")
    parser.add_argument(
        "--imgsz",
        "--img",
        "--img-size",
