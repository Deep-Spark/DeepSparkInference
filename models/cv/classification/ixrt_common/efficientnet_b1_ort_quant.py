"""
EfficientNet-B1 专用 ORT 静态量化脚本

核心问题与解决方案：
1. EfficientNet-B1 的 SE 模块输出分布极度不对称（范围如 [-0.27, 90+]，
...
2. Root Cause: Conv nodes receiving SE-attended tensor inputs have large activation
   ...
   distributions that suffer catastrophic precision loss under symmetric INT8.
   Calibration method (Entropy/MinMax/Percentile) does NOT help—all converge to the
   same scale because the true distribution spans the full range.

3. Solution: Exclude the 17 Conv nodes with activation scale > 0.4 from INT8
   quantization. They run in FP16, preserving SE precision. The remaining 98 Conv
   nodes are quantized with INT8. This mixed-precision approach achieves 68.92%
   IxRT accuracy (target: 65.1%).
...
5. IxRT compatibility: ActivationSymmetric=True (from quantize_with_torch_data)
   ensures all activation zero_points are 0, required by IxRT's symmetric quantization.

6. B1 has more MBConv blocks than B0 (92 Sigmoid nodes vs 65), resulting in 17
   problematic nodes (vs 13 for B0), but the same Entropy + batch=32 strategy applies.

Validation results (IxRT, 50000 val images, batch=32):
  Acc@1: 68.92% (target: 65.1%) ✓ PASS
"""
...
# Excluding them runs these 17 Conv nodes in FP16, preserving SE module precision.
# Identified via Entropy calibration scale analysis on EfficientNet_b1_sim.onnx.
# Achieves IxRT Acc@1: 68.92% (target: 65.1%) with 98 Conv nodes remaining in INT8.
_NODES_TO_EXCLUDE = [
    "Conv_98",    # scale=0.5322
    "Conv_167",   # scale=0.4250
    "Conv_179",   # scale=0.5430
    "Conv_240",   # scale=0.4938
    "Conv_250",   # scale=0.5057
    "Conv_251",   # scale=0.4849
    "Conv_324",   # scale=0.5285
    "Conv_397",   # scale=0.5623
    "Conv_469",   # scale=0.4619
    "Conv_542",   # scale=0.5200
    "Conv_615",   # scale=0.5162
    "Conv_687",   # scale=0.4259
    "Conv_760",   # scale=0.4061
    "Conv_906",   # scale=0.4791
    "Conv_1124",  # scale=0.4289
    "Conv_1197",  # scale=0.4462
    "Conv_1561",  # scale=0.4467
]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="EfficientNet-B1 专用 ORT INT8 量化工具（仅量化 Conv，batch=32 校准）"
    )
...
def main():
    ...
    try:
        ...
        # _NODES_TO_EXCLUDE 中的 17 个 Conv 节点以 FP16 运行，避免 SE 输出的对称量化精度损失
        quantize_with_torch_data(
