...
batchsize=32
model_path="wide_resnet50.onnx"
quantized_model_path="wide_resnet50_int8.onnx"
...
python3 ${RUN_DIR}build_engine.py                      \
    ...
    --precision int8                         \
    --engine_path wide_resnet50_bs_${batchsize}_int8.so

# inference
python3 ${RUN_DIR}inference.py                                  \
    --engine wide_resnet50_bs_${batchsize}_int8.so    \
    --batchsize ${batchsize}                          \
    --input_name input                                \
    --datasets ${datasets_path}                       \
    --perf_only True
