...
def main(config):
    ...
    if config.test_mode == "FPS":
        ...
        batch_data = np.random.rand(config.bsz, 3, config.imgsz, config.imgsz).astype(inputs[0]["dtype"])
        ...
        for bs in range(config.bsz):
            ...
            onnx_output = onnxruntime_infer(config.onnx_path, batch_data_)
            metricResult = compare(onnx_output, output[bs,:])
        print(metricResult)
