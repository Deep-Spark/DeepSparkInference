...
def argsparser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to COCO dataset root (e.g., /home/datasets/coco)")
    parser.add_argument(
        "--batch_size",
        type=int,
        default=1,
...
def load_predictor(
    ...
    if device == "GPU":
        ...
        precision_map = {
            ...
            # Note: Int8 is only effective when use_trt=True
            "int8": Config.Precision.Int8,
        }
        gpu_precision = precision_map.get(precision, Config.Precision.Float32)
        
        config.enable_use_gpu(200, 0, gpu_precision)
...
def eval(predictor, val_loader, metric, rerun_flag=False):
    ...
    time_min = float("inf")
    time_max = float("-inf")
    sample_nums = len(val_loader.dataset)
    input_names = predictor.get_input_names()
    output_names = predictor.get_output_names()
    boxes_tensor = predictor.get_output_handle(output_names[0])
    if FLAGS.include_nms:
    ...
    for batch_id, data in enumerate(val_loader):
        ...
        if batch_id % 100 == 0:
            ...
            sys.stdout.flush()
    if FLAGS.perf_only:
        fps = sample_nums / predict_time if predict_time > 0 else 0.0
        print("[Benchmark] FPS: {}".format(round(fps, 2)))
    else:
        metric.accumulate()
        metric.log()
...
def main():
    ...
    if FLAGS.image_file:
        ...
        reader_cfg["EvalDataset"].dataset_dir = FLAGS.dataset_dir
        reader_cfg["EvalReader"]["batch_size"] = FLAGS.batch_size
