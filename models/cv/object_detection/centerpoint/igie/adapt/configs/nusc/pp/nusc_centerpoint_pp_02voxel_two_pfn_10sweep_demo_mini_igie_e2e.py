...
# model settings — single e2e SO (pt2 + Relax torch frontend)
model = dict(
    type="PointPillarsIGIEE2E",
    pretrained=None,
    reader=dict(
...
test_cfg = dict(
    ...
    out_size_factor=get_downsample_factor(model),
    voxel_size=[0.2, 0.2],
    e2e_engine_path="./pp_e2e_fp16_ixinfer.so",
)

# dataset settings
dataset_type = "NuScenesDataset"
nsweeps = 10
data_root = os.environ['NUSCENES_PATH']
...
train_anno = os.path.join(data_root, "dbinfos_train_10sweeps_withvelo.pkl")
val_anno = os.path.join(data_root, "infos_val_10sweeps_withvelo_filter_True.pkl")
test_anno = None

data = dict(
    samples_per_gpu=1,  # e2e SO is batch=1
    workers_per_gpu=8,
    train=dict(
        type=dataset_type,
        root_path=data_root,
