...
__global__ void RoIAlignForward(const int nthreads, const float* bottom_data, const float spatial_scale,
                                ...
                                const int pooled_width, const int sampling_ratio, const float4* bottom_rois,
                                float* top_data) {
    for (size_t index = blockIdx.x * blockDim.x + threadIdx.x; index < nthreads; index += blockDim.x * gridDim.x) {
        ...
        int n = index / pooled_width / pooled_height / channels;  // B*N
        ...
        const float* offset_bottom_data = bottom_data + static_cast<int>(c * height * width);
        ...
        for (int iy = 0; iy < roi_bin_grid_h; iy++) {
            ...
            for (int ix = 0; ix < roi_bin_grid_w; ix++) {
                ...
                float val = bilinear_interpolate(offset_bottom_data, height, width, y, x);
...
int32_t roiAlign(int32_t batchSize, const void* const* inputs, void* TRT_CONST_ENQUEUE* outputs,
    ...
    auto in_boxes = static_cast<const float4*>(inputs[0]);
    auto in_features = static_cast<const float*>(inputs[1]);

    int32_t nthreads = batchSize * num_proposals * out_channels * pooler_resolution * pooler_resolution;
    auto out_features = static_cast<float*>(outputs[0]);
    const int32_t max_threads = 1024;

    int32_t blocksPerGrid = ceil(static_cast<float>(nthreads) / max_threads);
    RoIAlignForward<<<blocksPerGrid, max_threads, 0, stream>>>(
        nthreads, in_features, spatial_scale, out_channels, feature_h, feature_w, pooler_resolution, pooler_resolution,
        sampling_ratio, in_boxes, out_features);
