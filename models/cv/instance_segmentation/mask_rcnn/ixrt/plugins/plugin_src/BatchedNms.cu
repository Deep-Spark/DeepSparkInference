...
__global__ void batched_nms_kernel(const int nms_method, const float threshold, const int num_detections,
...
int32_t batchedNms(int32_t nms_method, int32_t batch_size, const void *const *inputs, void *TRT_CONST_ENQUEUE *outputs,
    ...
    if (!workspace || !workspace_size) {
        ...
        workspace_size += get_size_aligned<int32_t>(count);  // indices
        workspace_size += get_size_aligned<int32_t>(count);  // indices_sorted
        workspace_size += get_size_aligned<float>(count);    // scores_sorted

        size_t temp_size_sort = 0;
        cub::DeviceRadixSort::SortPairsDescending(static_cast<void *>(nullptr), temp_size_sort,
                                                  static_cast<float *>(nullptr), static_cast<float *>(nullptr),
                                                  static_cast<int *>(nullptr), static_cast<int *>(nullptr), count);
    ...
    for (int i = 0; i < count; i++) indices_h[i] = i;
    ...
    for (int batch = 0; batch < batch_size; batch++) {
        ...
        cub::DeviceRadixSort::SortPairsDescending(workspace, workspace_size, in_scores, scores_sorted, indices,
                                                  indices_sorted, num_detections, 0, sizeof(*scores_sorted) * 8,
                                                  stream);
        ...
        batched_nms_kernel<<<num_per_thread, max_threads, 0, stream>>>(
            nms_method, nms_thresh, num_detections, indices_sorted, scores_sorted, in_classes, in_boxes);

        // Re-sort with updated scores
        cub::DeviceRadixSort::SortPairsDescending(workspace, workspace_size, scores_sorted, scores_sorted,
                                                  indices_sorted, indices, num_detections, 0,
                                                  sizeof(*scores_sorted) * 8, stream);

        // Gather filtered scores, boxes, classes
        num_detections = std::min(detections_per_im, num_detections);
        cudaMemcpyAsync(out_scores, scores_sorted, num_detections * sizeof *scores_sorted, cudaMemcpyDeviceToDevice,
                        stream);
