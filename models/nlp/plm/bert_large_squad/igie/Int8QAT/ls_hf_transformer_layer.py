...
from quantization import qat_mode, ptq_mode, disable_quant
...
def inject_ls_layer(model, training_args, model_args, config):
    if model_args.module_type == 2:
        from training_ops.torch_transformer_layers import (
    ...
    for i in range(config.num_hidden_layers):
        ...
        if model_args.module_type == 2:
            if model_args.enable_quant:
                model.bert.encoder.layer[i].apply(quant_type)
            else:
...
class LSBertPreTrainedModel(BertPreTrainedModel):
    ...
    def save_pretrained(self, *args, **kwargs):
        if self.model_args.module_type == 2:
            super().save_pretrained(*args, **kwargs)
        elif self.model_args.module_type == 1:
