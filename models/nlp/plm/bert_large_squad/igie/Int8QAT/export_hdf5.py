...
src_emb_mapping_dict = OrderedDict(
    {
        ...
        # qa linear 
        "qa_weights": "qa_outputs weight",
        "qa_bias": "qa_outputs bias"
    }
)
...
def extract_bert_weights(
    ...
    if model_dir.endswith(".safetensors"):
        state_dict = load_file(model_dir, device="cpu")
    elif model_dir.endswith(".bin"):
        state_dict = torch.load(model_dir, "cpu")
    else:
        print(f"Unsupported file formats!")
...
if __name__ == "__main__":
    ...
    head_number = head_number_model[args.model_name.lower()]
    pad_id = 0
    max_step = 50
