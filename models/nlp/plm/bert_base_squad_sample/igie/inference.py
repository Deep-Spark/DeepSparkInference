...
def igie_engine_init(engine_path):
    ...
    module.set_alloc_flag(False)
    return module, engine, context

def igie_infer(module, input_ids, segment_ids, input_mask):
    module.set_input("input_ids", input_ids)
    module.set_input("segment_ids", segment_ids)
    module.set_input("input_mask", input_mask)
    module.run()
...
if __name__ == '__main__':
    ...
    with open(args.engine, 'rb') as f:
        ...
        module, engine, context = igie_engine_init(args.engine)
        ...
        def inference(features, tokens):
            ...
            for feature_index, feature in enumerate(features):
                ...
                h_output = igie_infer(module, input_ids, segment_ids, input_mask)
        ...
        def inference_all_dynamic(features_list, squad_examples, sort_index, all_precision):
            ...
            for input_ids, segment_ids in tqdm(all_token_ids):
                ...
                input_mask = np.zeros(input_ids.shape, dtype=np.int32)
                # h_output = igie_infer(module, input_ids.ravel(), segment_ids.ravel(), input_mask.ravel())
                ...
                h_output = module.get_output(0)
                ## you can also construct your own output buffer. 
                #h_output = cuda.pagelocked_empty(tuple(context.get_binding_shape(binding_idx_offset + 3)), dtype=np.float16)
                #module.get_output(0, h_output)
