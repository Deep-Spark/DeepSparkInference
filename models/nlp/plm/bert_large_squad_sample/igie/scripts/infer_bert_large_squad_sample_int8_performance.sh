...
project_path=./
checkpoints_path=${project_path}/data/checkpoints/bert_large_squad/bert-large-uncased
datasets_path=${project_path}/data/datasets/bert_large_squad/

echo 'USE_TRT='${USE_TRT}
export USE_TRT=$USE_TRT

echo "Step1 Build Engine Int8(bert large squad)!"
if [[ ! -e ${checkpoints_path}/bert_large_int8_b${BSZ}.engine ]]; then
