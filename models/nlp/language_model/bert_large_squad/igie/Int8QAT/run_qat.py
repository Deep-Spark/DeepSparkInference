...
from ls_hf_transformer_layer import inject_ls_layer, LSBertForQuestionAnswering,disable_quant
from pytorch_quantization.nn.modules.tensor_quantizer import TensorQuantizer,calib
...
def main():
    ...
    # Training
    if training_args.do_train and not (model_args.quant_mode == "ptq"):
        checkpoint = None
    ...
    if model_args.quant_mode == "ptq":
        ...
        train_iter = trainer.get_train_dataloader()
        for step, batch in enumerate(train_iter):
            if step > 20:
                break
            batch = {i:batch[i].to("cuda:0") for i in batch}  # multi-gpu does scattering it-self
            start_logits, end_logits = model(**batch)
            model.bert.embeddings.apply(finish_calibration)
            for i in range(config.num_hidden_layers):
                model.bert.encoder.layer[i].apply(finish_calibration)
